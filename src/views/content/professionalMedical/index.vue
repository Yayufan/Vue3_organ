<!--  -->
<template>
  <section class="professional-medical-section">
    <el-card class="professional-medical-card">
      <h1>專業醫療文章管理</h1>
 ...
 * 這邊將數組放入,將articleCategoryId(PMI key)取出,賦予別名為id; name取出,賦予別名label方便後面通用組件顯示
 * 
 * @param arr 
 */
...
//專業醫療文章分頁列表
const professionalMedicalTable = reactive({})
//專業醫療文章類別
...
const getAllProfessionalMedicalCategory = async () => {
  ...
  console.log('這是在專業醫療文章獲取的類別', categoryTable)
}

//獲取專業醫療文章(分頁)
const getProfessionalMedical = async (page: number, size: number) => {
  let res = await getAllArticleByGroupByPaginationApi(GROUP, page, size)
  let transData = transFormPagination(res.data, "articleId")
  Object.assign(professionalMedicalTable, transData)
  console.log('獲取專業醫療文章', professionalMedicalTable)
}

//新增專業醫療文章
const addProfessionalMedical = async (data: any) => {
  let res = await addArticleApi(data)
  getAllProfessionalMedicalCategory()
  let currentPath = route.fullPath
  router.push(currentPath + '/' + res.data)
}

//刪除專業醫療文章
const deleteProfessionalMedical = async (id: number) => {
  await deleteArticleApi(id)
}

//批量刪除專業醫療文章
const batchDeleteProfessionalMedical = async (data: any) => {
